...
type Endpoint struct {
	...
	MergeOptions MergeOptions `json:"mergeOptions" db:"merge_options"`
}
...
func (c *Conn) GetRouteRequestData(projectName, routePath string) (endpoint Endpoint, err error) {
	stmt, err := c.Db.Preparex(fmt.Sprintf(`
		...
		merge_options_values(id, route_id, merge_header, merge_body, merge_query) AS 
		(
			SELECT id, route_id, merge_header, merge_body, merge_query FROM merge_options 
			WHERE route_id = (SELECT id FROM route_id)
		)
		...
		LEFT JOIN merge_options_values ON merge_options_values.route_id = route.id
		...
		GROUP BY route.id, merge_options_values.*;
	`, PG_CRYPT_KEY))
